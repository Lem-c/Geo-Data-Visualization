...
export default {
  ...
  methods: {
      ...
      this.markers.forEach(marker => {
        if (this.addedMarkers.length == 0 ||
          ...
          el.addEventListener('click', () => {
            ...
            this.loadForecast(marker.coordinates[0], marker.coordinates[1]);
          });
        }
      });
    },
    ...
    loadForecast(lat, lon) {
      const openWeatherURL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${this.openWeatherAPI}`;
      ...
      fetch(openWeatherURL)
        ...
        .then(data => {
          ...
          this.forecast = data.list.map(item => ({
            date: item.dt_txt,
            main: item.main,
